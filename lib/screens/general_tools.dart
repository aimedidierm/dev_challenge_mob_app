...
class GeneralTools extends StatefulWidget {
  const GeneralTools({super.key});

  @override
  State<GeneralTools> createState() => _GeneralToolsState();
}

class _GeneralToolsState extends State<GeneralTools> {
  bool _loading = false;
  ...
  Future<void> fetchData() async {
    ...
    final response = await http.get(Uri.parse(generalListToolsURL), headers: {
  ...
  Future<void> rejectTool(String id) async {
    String token = await getToken();
    final response = await http.get(Uri.parse(generalRejectURL + '$id'),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer $token'
        });
  ...
  Future<void> approveTool(String id) async {
    String token = await getToken();
    final response = await http.get(Uri.parse(generalApproveURL + '$id'),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer $token'
        });
