...
class FinanceAll extends StatefulWidget {
  const FinanceAll({super.key});

  @override
  State<FinanceAll> createState() => _FinanceAllState();
}

class _FinanceAllState extends State<FinanceAll> {
  bool _loading = false;
  ...
  Future<void> fetchData() async {
    ...
    final response = await http.get(Uri.parse(financeListURL), headers: {
  ...
  Future<void> rejectTool(String id) async {
    String token = await getToken();
    final response = await http.get(Uri.parse(financeRejectURL + '$id'),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer $token'
        });
  ...
  Future<void> approveTool(String id) async {
    String token = await getToken();
    final response = await http.get(Uri.parse(financeApproveURL + '$id'),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer $token'
        });
